...
describe('BGCL', function () {
  it('splits keys that can be recombined', function () {
    const bgcl = new BGCLFixedKey(testKey);
    ...
    const result = bgcl.genSplitKey(params);
    const decrypted = _.map(result.seedShares, (v, i) => {
    ...
    const seed = secrets.combine(decrypted);
